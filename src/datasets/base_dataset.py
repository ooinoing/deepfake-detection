...
# base dataset class for loading audio files and their metadata
class SimpleAudioFakeDataset(Dataset):
    ...
    # Splits the list or DataFrame of samples into subsets based on a partition ratio
    def split_samples(self, samples_list):
        # if dataframe : change to list
    ...
    # Converts the samples DataFrame -> list of tuples, easier to iterate
    def df2tuples(self):
        tuple_samples = []
    ...
    # how to access individual items from the dataset
    def __getitem__(self, index) -> T_co:
        # Loads the audio file, applies preprocessing

        ### 1. Checking Sample Type if DataFrame
        ...
        # 2. Loading and Preprocessing the Audio
        # loads the audio file + optionally normalize
        waveform, sample_rate = torchaudio.load(path, normalize=APPLY_NORMALIZATION)
        real_sec_length = len(waveform[0]) / sample_rate
        #  actual duration of the audio clip in seconds, by dividing the number of samples in the waveform by the sample rate.
