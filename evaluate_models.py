...
from src import metrics, commons  # Custom modules for metrics and common functions
from src.models import models  # Custom module for loading model architectures
from src.datasets.base_dataset import SimpleAudioFakeDataset
from src.datasets.in_the_wild_dataset import InTheWildDataset


# Function to load the dataset based on given paths and optional limit on the amount of data
def get_dataset(
    datasets_paths: List[Union[Path, str]], amount_to_use: Optional[int]
) -> SimpleAudioFakeDataset:
    data_val = InTheWildDataset(subset="test", path=datasets_paths[0])
    return data_val
...
def evaluate_nn(
    ...
    amount_to_use: Optional[int] = None,  # Optional limit on the amount of data to use
    batch_size: int = 8,
):
    logging.info("Loading data...")
    """
    logging.info(f" model_paths : {model_config["checkpoint"].get("path", [])}")
    logging.info(f" datasets_paths : {datasets_paths}")
    logging.info(f" model_config :{model_config["model"]}")
    """

    model_name, model_parameters = model_config["name"], model_config["parameters"]

    logging.info(f"model_name :{model_name}")
    logging.info(f"model_parameters :{model_parameters}")
    # Load the specified model  with given parameters and move it to device
    ...
    # Initialize the DataLoader for batch processing
    ...
    # Initialize tensors for predictions and labels
    y_pred = torch.Tensor([]).to(device)
    y = torch.Tensor([]).to(device)
    y_pred_label = torch.Tensor([]).to(device)

    model.eval()  # Set the model to evaluation mode
    # code block is meant for infegit remote remove originrence or evaluation not training
    with torch.no_grad():  # Disable gradient calculation for efficiency
        ...
        for i, (batch_x, _, batch_y) in enumerate(test_loader):
            if i % 10 == 0:
                ...
                # Make predictions and apply sigmoid to get probabilities
                ...
                # Accumulate predictions and labels for later analysis
    ...
    y_for_eer = 1 - y  # Invert labels for EER calculation
...
def parse_args():
    ...
    print(os.getcwd())
    DATASET_PATH = "../datasets/release_in_the_wild"

    parser.add_argument("--datasets_paths", type=str, default=DATASET_PATH)
